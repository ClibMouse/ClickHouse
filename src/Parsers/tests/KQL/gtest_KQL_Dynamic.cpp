...
INSTANTIATE_TEST_SUITE_P(ParserKQLQuery_DynamicRegex, ParserRegexTest,
    ::testing::Combine(
        ...
        {
            "print zip(A, B)",
            R"(SELECT arrayMap\(t -> \[untuple\(t\)\], arrayZip\(arrayResize\(arg0_\d+, arrayMax\(\[length\(if\(match\(toTypeName\(A\), 'Array\\\\\(Nullable\\\\\(\.\*\\\\\)\\\\\)'\), A, CAST\(A, concat\('Array\(', extract\(toTypeName\(if\(length\(A\) = 0, \[NULL\], A\)\), 'Array\\\\\(\(\.\*\)\\\\\)'\), '\)'\)\)\) AS arg0_\d+\), length\(if\(match\(toTypeName\(B\), 'Array\\\\\(Nullable\\\\\(\.\*\\\\\)\\\\\)'\), B, CAST\(B, concat\('Array\(', extract\(toTypeName\(if\(length\(B\) = 0, \[NULL\], B\)\), 'Array\\\\\(\(\.\*\)\\\\\)'\), '\)'\)\)\) AS arg1_\d+\)\]\) AS max_length_\d+, NULL\), arrayResize\(arg1_\d+, max_length_\d+, NULL\)\)\))"
        },
        {
            "print zip(A, B, C)",
            R"(SELECT arrayMap\(t -> \[untuple\(t\)\], arrayZip\(arrayResize\(arg0_\d+, arrayMax\(\[length\(if\(match\(toTypeName\(A\), 'Array\\\\\(Nullable\\\\\(\.\*\\\\\)\\\\\)'\), A, CAST\(A, concat\('Array\(', extract\(toTypeName\(if\(length\(A\) = 0, \[NULL\], A\)\), 'Array\\\\\(\(\.\*\)\\\\\)'\), '\)'\)\)\) AS arg0_\d+\), length\(if\(match\(toTypeName\(B\), 'Array\\\\\(Nullable\\\\\(\.\*\\\\\)\\\\\)'\), B, CAST\(B, concat\('Array\(', extract\(toTypeName\(if\(length\(B\) = 0, \[NULL\], B\)\), 'Array\\\\\(\(\.\*\)\\\\\)'\), '\)'\)\)\) AS arg1_\d+\), length\(if\(match\(toTypeName\(C\), 'Array\\\\\(Nullable\\\\\(\.\*\\\\\)\\\\\)'\), C, CAST\(C, concat\('Array\(', extract\(toTypeName\(if\(length\(C\) = 0, \[NULL\], C\)\), 'Array\\\\\(\(\.\*\)\\\\\)'\), '\)'\)\)\) AS arg2_\d+\)\]\) AS max_length_\d+, NULL\), arrayResize\(arg1_\d+, max_length_\d+, NULL\), arrayResize\(arg2_\d+, max_length_\d+, NULL\)\)\))"
        }
})));
