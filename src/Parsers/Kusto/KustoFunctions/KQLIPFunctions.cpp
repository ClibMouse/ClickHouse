...
bool Ipv4Compare::convertImpl(String & out, IParser::Pos & pos)
{
    ...
    out = std::format(
        ...
        "toUInt8(min2({4}, min2(assumeNotNull(lhs_mask_{5}), assumeNotNull(rhs_mask_{5})))) as mask_{5}), 1)::UInt32)"
        "   - toInt64(tupleElement(IPv4CIDRToRange(assumeNotNull(rhs_ip_{5}), mask_{5}), 1)::UInt32)))",
...
bool Ipv4IsMatch::convertImpl(String & out, IParser::Pos & pos)
{
    ...
    out = std::format("equals({}, 0)", kqlCallToExpression("ipv4_compare", {lhs, rhs, mask.value_or("32")}, pos.max_depth, pos.max_backtracks));
    return true;
}
...
bool ParseIpv4::convertImpl(String & out, IParser::Pos & pos)
{
    ...
    out = std::format(
        ...
        "tupleElement(IPv4CIDRToRange(assumeNotNull(ip_{1}), assumeNotNull(mask_{1})), 1)::UInt32, null)",
        ip_address,
        generateUniqueIdentifier());
    return true;
}
...
bool Ipv6IsMatch::convertImpl(String & out, IParser::Pos & pos)
{
    ...
    const auto rhs = getArgument(function_name, pos, ArgumentState::Raw);
    const auto mask = getOptionalArgument(function_name, pos, ArgumentState::Raw);
    out = std::format("equals({}, 0)", kqlCallToExpression("ipv6_compare", {lhs, rhs, mask.value_or("128")}, pos.max_depth, pos.max_backtracks));
    return true;
}
